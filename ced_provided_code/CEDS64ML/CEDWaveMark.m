...
classdef CEDWaveMark < CEDMarker
    properties (GetAccess = public, SetAccess = private)
        m_Data;
    end
    
    methods
        function obj = CEDWaveMark(Time, Code1, Code2, Code3, Code4, Wave)
            % class constructor
            obj = obj@CEDMarker();
            obj.m_Data = zeros(0,0,'int16');
            if(nargin > 0)
            ...
            if(nargin > 5)
                if (isnumeric(Wave) && ismatrix(Wave))
                    obj.m_Data = int16(Wave);
                end
            end
        end
        % access functions
